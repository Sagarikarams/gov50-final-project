...
```
```{r}
perc_chng_gasoline <- gasoline_cpi
perc_chng_gasoline$year <- year(gasoline_cpi$Date)
perc_chng_gasoline$month <- month(gasoline_cpi$Date)

perc_chng_gasoline <- perc_chng_gasoline|>
  group_by(year)|>
  filter(month == 1)|>
  select(year, Index)
...
```{r}
perc_chng_gasoline$perc_inc <- (perc_chng_gasoline$Index - lag(perc_chng_gasoline$Index))/ lag(perc_chng_gasoline$Index)*100

perc_chng_gasoline

knitr::kable(perc_chng_gasoline, col.names = c("Year", "CPI", "Percent_Change_CPI"))
```
```{r}
perc_chng_gasoline$year <- as.Date(as.character(perc_chng_gasoline$year), format = "%Y")
ggplot(perc_chng_gasoline, aes(x = year, y = perc_inc)) + geom_line() + labs(x = "Year", y = "Percent Change in CPI", title = "12-month percent change in CPI for gasoline, Jan. 2013 - Jan. 2022") 
```

In the above table I have calculated 12-month percent change in CPI for gasoline. The line plot is also obtained. We can see that CPI decreased in 2016, 2020 and 2022. But the CPI is increasing towards the end of 2022.
